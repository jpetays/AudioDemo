...
namespace Prg.Window
{
    ...
    public class WindowLoader : MonoBehaviour
    {
        ...
        private const string Tp2 = "Reset Window Stack (e.g. for main menu)";
        ...
        private static bool _isTagValidated;

        public static void ValidateWindowLoaderTag(string tag)
        {
            // NOTE that we can not use autogenerated tags by GameConstantsGenerator
            // because its namespace can be inaccessible to us.
            if (tag != TagName)
            {
                Debug.LogWarning("*");
                Debug.LogWarning($"* WindowLoader requires that its tag is same as given tag: '{tag}'");
                Debug.LogWarning("*");
                Assert.AreEqual(TagName, tag, $"invalid WindowLoader tag: {tag}");
            }
        ...
        private void Awake()
        {
            if (!_isTagValidated)
            {
                Debug.LogWarning("*");
                Debug.LogWarning(
                    $"* WindowLoader requires that its tag is: '{TagName}' and ValidateWindowLoaderTag is called at start");
                Debug.LogWarning("*");
                Assert.IsTrue(_isTagValidated,
                    "WindowLoader tag is not validated using WindowLoader.ValidateWindowLoaderTag()");
            }
